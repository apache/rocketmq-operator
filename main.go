...
func init() {
	...
	utilruntime.Must(rocketmqv1alpha1.AddToScheme(scheme))
...
func main() {
	...
	if err := broker.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to add broker controller to manager")
		os.Exit(1)
	}

	if err := console.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to add console controller to manager")
		os.Exit(1)
	}
